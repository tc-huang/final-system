...
```bash
state-function$ sam build --use-container
```
...
```bash
state-function$ sam logs -n StockCheckerFunction --stack-name state-function --tail
```
...
```bash
state-function$ pip install -r tests/requirements.txt --user
# unit test
state-function$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
state-function$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name state-function
```
